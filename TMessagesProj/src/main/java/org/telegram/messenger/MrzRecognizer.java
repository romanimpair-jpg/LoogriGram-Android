...
public class MrzRecognizer {
	...
	private static Result recognizeBarcode(Bitmap bitmap) {
		// LoogriGram: no barcode reading. This used Google's Mobile Vision
		// barcode detector, which has built-in parsers for North American
		// driver licence barcodes and the PDF417 on newer Russian ones. The
		// detector is not bundled - Play Services downloads it - so it never
		// worked here.
		//
		// recognizeMRZ above is untouched and is not Google's: it is
		// Telegram's own pure-Java reader for the machine-readable zone on
		// passports and ID cards. That still works, so document scanning is
		// not lost, only the driver-licence barcode shortcut. recognize()
		// already treats a null from here as "not a barcode, try the MRZ".
		return null;
	}
