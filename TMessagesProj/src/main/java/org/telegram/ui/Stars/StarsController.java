...
public class StarsController {
    ...
    public ArrayList<TL_stars.TL_starsTopupOption> getOptions() {
        ...
        ConnectionsManager.getInstance(currentAccount).sendRequest(new TL_stars.TL_payments_getStarsTopupOptions(), (res, err) -> AndroidUtilities.runOnUIThread(() -> {
            // LoogriGram: Star options keep the prices the server sends. Upstream
            // queued every option with a store_product and re-priced it from Play
            // product details, marking each loadingStorePrice until the query came
            // back; with useInvoiceBilling always true nothing was ever queued, so
            // the queue, the query and the missingStorePrice fallback are gone.
    ...
    public void buy(
        ...
        // LoogriGram: the Play Billing purchase for this ran from here - query
        // the product, launch the billing flow, then confirm the topup against
        // payments.canPurchaseStore. It was already unreachable: the invoice
        // branch above is taken whenever useInvoiceBilling is true, which is
        // always, and the only way past it is a server flag disabling invoice
        // billing for the peer - in which case the isReady() guard directly
